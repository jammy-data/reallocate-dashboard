...
from utils.visualization_utils import show_kpi_data
from utils.config import PILOTS_DIR
...
def render(kpi_ids):
    ...
    # 👇 Switch depending on KPI id
    if selected_kpi["id"] == 2:  # "Number of Participants in Co-Creation Workshops"
        file_path = PILOTS_DIR/"barcelona_pilot_1_kpi_participants.csv"
        show_kpi_data(file_path, kpi_title, kpi_description, kpi_unit)

    else:
        # Keep old dummy plot for other KPIs
